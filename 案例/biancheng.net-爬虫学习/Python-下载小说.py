...
def get_text(href):
    #创建请求对象
    requests = request_html(href)
    content = urllib.request.urlopen(requests).read().decode('utf8')
...
def run():
    ...
    #构建请求对象
    requests = request_html(url)
    # 发送请求，得到响应，转换为HTML对象
    html = urllib.request.urlopen(requests).read().decode('utf8')
    #解析内容
    parse_html(html,f)
    #关闭文件
    f.close()
